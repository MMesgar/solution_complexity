...
from optimizers import sgd_updates_adadelta
import activations
...
def train_conv_net(rng,
    ...
    # sent_h is half of the length of each instance, because
    ...
    # build filters for convolution and pooling
    # filter width is equal to the word emebedding or sentence width
    filter_w = sent_w
    filter_shapes = []
    pool_sizes = []
    for filter_h in filter_hs:
        filter_shapes.append((feature_maps, 1, filter_h, filter_w))
        pool_sizes.append((sent_h-filter_h+1, sent_w-filter_w+1))
    
    #define parametes of model
    parameters = [("sentence shape",sent_h,sent_w),
    ...
    for i in xrange(len(filter_hs)):
        ...
        conv_layer1 = LeNetConvPoolLayer(rng,
                                        input=layer0_input1,
                                        image_shape=(batch_size, 1, sent_h, sent_w),
                                        filter_shape=filter_shape,
                                        poolsize=pool_size,
                                        non_linear=conv_non_linear)
        conv_layer2 = LeNetConvPoolLayer(rng,
                                        input=layer0_input2,
                                        image_shape=(batch_size, 1, sent_h, sent_w),
                                        filter_shape=filter_shape,
                                        poolsize=pool_size,
                                        non_linear=conv_non_linear)
        conv_layer3 = LeNetConvPoolLayer(rng,
                                        input=layer0_input3,
                                        image_shape=(batch_size, 1, sent_h, sent_w),
                                        filter_shape=filter_shape,
                                        poolsize=pool_size,
        ...
        layer1_input1 = conv_layer1.output.flatten(2)
        layer1_input2 = conv_layer2.output.flatten(2)
        layer1_input3 = conv_layer3.output.flatten(2)
        
        conv_layers1.append(conv_layer1)
        conv_layers2.append(conv_layer2)
        conv_layers3.append(conv_layer3)
        
        layer1_inputs1.append(layer1_input1)
        layer1_inputs2.append(layer1_input2)
        layer1_inputs3.append(layer1_input3)
        
    layer1_input1 = T.concatenate(layer1_inputs1,1)
    layer1_input2 = T.concatenate(layer1_inputs2,1)
    layer1_input3 =T.concatenate(layer1_inputs3,1)
    
    difference  = layer1_input3 - layer1_input2
       
    layer1_input = T.concatenate([layer1_input1, layer1_input2, layer1_input3,difference],1) 

    hidden_units[0] = (4)*(feature_maps*len(filter_hs))
    
    #layer1_input = T.concatenate([layer1_input1,difference],1)
    #hidden_units[0]= 2*(feature_maps*len(filter_hs))
    ...
    for conv_layer in conv_layers1:
        params += conv_layer.params
    for conv_layer in conv_layers2:
        params += conv_layer.params
    for conv_layer in conv_layers3:
        params += conv_layer.params
    ...
    #cost = classifier.hinge(y)
    #dropout_cost = classifier.dropout_hinge(y)
 ...
 #   train_set_x = new_data[0][:n_train_batches*batch_size,:]
 #   train_set_y = new_data[1][:n_train_batches*batch_size]
    train_set_x = new_data[0]
    train_set_y = new_data[1]

#    val_set_x = new_data[0][n_train_batches*batch_size:,:]
#    val_set_y = new_data[1][n_train_batches*batch_size:]
    ...
    # compute the conv_layer problem and solutoin
    for conv_layer in conv_layers1:
        test_layer0_output1 = conv_layer.predict(test_layer0_input1, test_size)
        test_pred_layers1.append(test_layer0_output1.flatten(2))
    for conv_layer in conv_layers2:
        test_layer0_output2 = conv_layer.predict(test_layer0_input2, test_size)
        test_pred_layers2.append(test_layer0_output2.flatten(2))
    for conv_layer in conv_layers3:
        test_layer0_output3 = conv_layer.predict(test_layer0_input3, test_size)
        test_pred_layers3.append(test_layer0_output3.flatten(2))    
        
    # conclude the inputs of the next layer
    test_layer1_input1 = T.concatenate(test_pred_layers1, 1)
    test_layer1_input2 = T.concatenate(test_pred_layers2, 1)
    test_layer1_input3 = T.concatenate(test_pred_layers3,1)
    
    difference = test_layer1_input3  -  test_layer1_input2
    test_layer1_input = T.concatenate([test_layer1_input1,test_layer1_input2,test_layer1_input3, difference],1)
    #test_layer1_input = T.concatenate([test_layer1_input1,difference],1)
...
if __name__=="__main__":
	...
	filter_sizes= [4,5]
        ...
        perf, errors, labels_prob = train_conv_net(rng,
                              datasets,
                              U,
                              lr_decay=0.9,
