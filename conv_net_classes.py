...
class ranking_layer:
    def __init__(self, input, activation):
        ...
        #compute the score between p and s_pos
        self.score_pos= T.dot(self.p, self.s_pos)
        
        #compute the score between p and s_neg
        self.score_neg = T.dot(self.p, self.s_neg)
        
        #compute the hing loss
        self.loss =T.mean(T.maximum(0, 1 - self.score_pos + self.score_neg))


class MLP_prob:
    '''
        This class is output layer with only one output node.
        It differes with MLP in the output's type. Here is the probability
    '''
    def __init__(self, rng, input, n_in, n_hidden):
        ...
        self.errors = self.logRegressionLayer.error_MSE
...
def _dropout_from_layer(srng, layer, p):
...
class MLPDropout(object):
    ...
    def __init__(self,rng,input,layer_sizes,dropout_rates,activations,use_bias=True):
        ...
        # the objective. or Hinge
        self.dropout_hinge = self.dropout_layers[-1].hinge
        self.hinge = self.layers[-1].hinge
...
class LogisticRegression(object):
    ...
    def error_MSE(self,y):
        # check if y has same dimension of y_pred
        if y.ndim != self.y_pred.ndim:
            raise TypeError('y should have the same shape as self.y_pred',
                ('y', target.type, 'y_pred', self.y_pred.type))
        # check if y is of the correct datatype
        if y.dtype.startswith('float32'):
    ...
    def hinge(self,y):
        """
        This funcution computes the hinge (max-margine) loss function
        The implementation is inspired from https://github.com/fchollet/keras/blob/master/keras/losses.py
        """
        return T.mean(1.0-(self.p_y_given_x[T.arange(y.shape[0]), y]))
